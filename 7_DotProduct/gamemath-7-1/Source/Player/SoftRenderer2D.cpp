...
void SoftRenderer::Update2D(float InDeltaSeconds)
{
	...
	// 시야각 계산
	static float halfFocCos = cosf(Math::Deg2Rad(fovAngle * 0.5f));

	elapsedTime = Math::Clamp(elapsedTime + InDeltaSeconds, 0.f, duration);

	// 지정한 시간이 경과하면 새로운 이동 지점을 랜덤하게 설정 - 목표물 이동
	...
	// 물체의 최종 상태 설정
	if (v.Dot(f) >= halfFocCos) // 해당 내적값이 halfFovCos 보다 크다면 시야 각안에 들어와 있다.
	{
