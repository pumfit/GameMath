...
void SoftRenderer::Update2D(float InDeltaSeconds)
{
	...
	static std::uniform_real_distribution<float> randomY(-200.f, 200.0f);
	// 선을 구성하는 두 점의 Y 값을 랜덤하게 가져온다.

	elapsedTime = Math::Clamp(elapsedTime + InDeltaSeconds, 0.f, duration);
	if (elapsedTime == duration) // 경과 시간이 지난 경우에
	{
...
void SoftRenderer::Render2D()
{
	...
	Vector2 projV = unitV * (u.Dot(unitV)); // 투영 공식 사용
	Vector2 projectedPoint = lineStart + projV;
	float distance = (projectedPoint - point).Size();// 최단 거리 계산
