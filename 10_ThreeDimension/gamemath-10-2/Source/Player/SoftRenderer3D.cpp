...
void SoftRenderer::Update3D(float InDeltaSeconds)
{
	...
	{
		r.Pitch += input.GetAxis(InputAxis::ZAxis) * rotateSpeed * InDeltaSeconds; // Z 축 입력
	}
	r.Roll += input.GetAxis(InputAxis::XAxis) * rotateSpeed * InDeltaSeconds; // X 축 입력
	r.Yaw += input.GetAxis(InputAxis::YAxis) * rotateSpeed * InDeltaSeconds; // Y 축 입력
