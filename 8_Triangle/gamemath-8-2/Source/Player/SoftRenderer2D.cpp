...
void SoftRenderer::Render2D()
{
	...
	for (size_t ti = 0; ti < triangleCount; ++ti)
	{
		...
		//삼각형을 구성하는 세 점을 배열에 보관한다.
		std::array<Vertex2D, 3> triangleVector = { vertices[indices[bi]] , vertices[indices[bi + 1]], vertices[indices[bi + 2]] };

		// 세 점 사이의 최소 x,y 값
		Vector2 minPos(Math::Min3(triangleVector[0].Position.X, triangleVector[1].Position.X, triangleVector[2].Position.X), Math::Min3(triangleVector[0].Position.Y, triangleVector[1].Position.Y, triangleVector[2].Position.Y));
		Vector2 maxPos(Math::Max3(triangleVector[0].Position.X, triangleVector[1].Position.X, triangleVector[2].Position.X), Math::Max3(triangleVector[0].Position.Y, triangleVector[1].Position.Y, triangleVector[2].Position.Y));

		// u : 1->0 ,v: 2->0 벡터를 구한다.
		Vector2 u = triangleVector[1].Position - triangleVector[0].Position;
		Vector2 v = triangleVector[2].Position - triangleVector[0].Position;

		// 공통 분모 ( uu * vv - uv * uv ) 내적을 사용하여 무게중심좌표를 구할 분모 값을 구한다.
		float udotv = u.Dot(v);
		float vdotv = v.Dot(v);
		float udotu = u.Dot(u);
		float denominator = udotv * udotv - vdotv * udotu;

		// 분모 denominator 가 0이면 퇴화삼각형이므로 그리지 않음
		if (denominator == 0.0f)
		{
			continue;
		}
		...
		for (int x = lowerLeftPoint.X; x <= upperRightPoint.X; ++x)
		{
			for (int y = upperRightPoint.Y; y <= lowerLeftPoint.Y; ++y)
			{
				...
				// 첫번 쨰 점에서 확인 할 점으로의 벡터를 구하고 분자 내적 값을 구한다.
				Vector2 w = pointToTest - triangleVector[0].Position;
				float wdotu = w.Dot(u);
				float wdotv = w.Dot(v);
				...
				// 컨벡스 조건을 만족할 때만 점 찍기 ,  모든 값이 0~1 사이인 경우 
