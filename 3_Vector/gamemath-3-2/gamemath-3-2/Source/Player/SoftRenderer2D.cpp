...
void SoftRenderer::Update2D(float InDeltaSeconds)
{
	...
	Vector2 inputVector = Vector2(input.GetAxis(InputAxis::XAxis), input.GetAxis(InputAxis::YAxis)).GetNormalize(); // 정규화 진행
...
void SoftRenderer::Render2D()
{
	...
	if (circles.empty())// 비어 있다면 최초 1회 반지름보다 긴 벡터를 모아 컨테이너에 넣는다.
	{
		for (float x = -radius; x <= radius; ++x)
		{
			for (float y = -radius; y <= radius; ++y)
			{
				...
				float squaredLength = pointToTest.SizeSquared(); // SizeSquared = x^2 + y^2
	...
	{
		// 각 점 위치에 현재 중점 값을 더해 붉은 색으로 칠한다.
